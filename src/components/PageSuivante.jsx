...
function PageSuivante({ onBack }) {
  ...
  const generateCompletePDFFromPageSuivante = async (firstPageData) => {
    ...
    // Page 4 - Graphiques circulaires des statistiques
    doc.addPage()
    ...
    const statsProduitsExploitation = totalProduits
    ...
    const statsChargesExploitation = totalCharges
    ...
    const produitsData = [
      statsProduitsExploitation,
      statsProduitsFinanciers,
      statsProduitsExceptionnels
    ]
    const produitsLabels = ['Produits d\'exploitation', 'Produits financiers', 'Produits exceptionnels']
    const produitsColors = [
      [52, 152, 219], // Bleu
      [46, 204, 113], // Vert
      [155, 89, 182]  // Violet
    ]
    ...
    if (totalProduitsAll > 0) {
      // Dessiner le graphique circulaire (donut chart)
      ...
      produitsData.forEach((value, index) => {
        if (value > 0) {
          ...
          // Dessiner plusieurs lignes radiales pour remplir le secteur
          ...
          // Dessiner les lignes radiales de séparation
          ...
          // Ajouter le pourcentage au centre du secteur
      ...
      // Dessiner les cercles de contour
      doc.setLineWidth(2)
      doc.setDrawColor(0, 0, 0)
    ...
    // Légende pour les produits
    yPosition = chart1Y + chart1Radius + 20
    doc.setFontSize(9)
    doc.setFont('helvetica', 'normal')
    ...
    produitsData.forEach((value, index) => {
      if (value > 0) {
        ...
        // Carré coloré
        doc.setFillColor(produitsColors[index][0], produitsColors[index][1], produitsColors[index][2])
        doc.rect(marginLeft, yPosition - 2, 3, 3, 'F')
        doc.setTextColor(0, 0, 0)
        doc.text(`${produitsLabels[index]}: ${percent}%`, marginLeft + 6, yPosition)
        yPosition += 5
      }
    })
    ...
    doc.setFontSize(14)
    doc.setFont('helvetica', 'bold')
    doc.text('Répartition des Charges', pageWidth / 2, yPosition, { align: 'center' })
    yPosition += 10
    
    const chart2X = pageWidth / 2
    const chart2Y = yPosition + 25
    const chart2Radius = 25
    
    const chargesData = [
      statsChargesExploitation,
      statsChargesFinancieres,
      statsChargesExceptionnelles
    ]
    ...
    // Légende pour les charges
    yPosition = chart2Y + chart2Radius + 20
    doc.setFontSize(9)
    doc.setFont('helvetica', 'normal')
    ...
    chargesData.forEach((value, index) => {
      if (value > 0) {
        ...
        doc.rect(marginLeft, yPosition - 2, 3, 3, 'F')
        doc.setTextColor(0, 0, 0)
        doc.text(`${chargesLabels[index]}: ${percent}%`, marginLeft + 6, yPosition)
        yPosition += 5
      }
    })
